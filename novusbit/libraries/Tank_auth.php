...
class Tank_auth
{
			...
			if ($cookie = get_cookie($this->ci->config->item('autologin_cookie_name', 'tank_auth'), TRUE)) {
				...
				if (isset($data['key']) AND isset($data['user_id'])) {
					...
					if (!is_null($user = $this->ci->user_autologin->get($data['user_id'], md5($data['key'])))) {
						...
						set_cookie($this->ci->config->item('autologin_cookie_name', 'tank_auth'),
							   $cookie,
							   $this->ci->config->item('autologin_cookie_life', 'tank_auth'));
	...
	public	function validate_logged_user( &$data )
	{
		
		$current_logged_status = $this->is_logged_in();
	
                if ( $current_logged_status  )
                	$current_logged_username = $this->get_username();
                ...
                {
	                $data['auth'] = $data['owner'] = $this->get_username();
                }
